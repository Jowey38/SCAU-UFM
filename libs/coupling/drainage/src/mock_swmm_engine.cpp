...
void validate_finite(core::Real value, const char* name) {
...
void MockSwmmEngine::step(core::Real dt_swmm) {
...
core::Real MockSwmmEngine::get_node_head(std::size_t node_id) const {
    validate_node(node_id);
    return node_heads_[node_id];
}

core::Real MockSwmmEngine::get_node_lateral_inflow(std::size_t node_id) const {
    validate_node(node_id);
    return node_lateral_inflows_[node_id];
}

void MockSwmmEngine::set_node_lateral_inflow(std::size_t node_id, core::Real q) {
    validate_node(node_id);
    validate_finite(q, "SWMM lateral inflow must be finite");
    node_lateral_inflows_[node_id] = q;
}

core::Real MockSwmmEngine::get_link_flow(std::size_t link_id) const {
...
void MockSwmmEngine::set_node_head(std::size_t node_id, core::Real head) {
    validate_node(node_id);
...
void MockSwmmEngine::set_link_flow(std::size_t link_id, core::Real flow) {
    validate_link(link_id);
    validate_finite(flow, "SWMM link flow must be finite");
    link_flows_[link_id] = flow;
}

core::Real MockSwmmEngine::elapsed_time() const {
