...
TEST(TriCouplingMultiStep, ThousandStepBalancedExchangeHasNoMassDriftOrStaleLateral) {
    ...
    swmm.set_node_overflow_fixture(11, q);
    const TriCouplingStepConfig config{
        .surface_drainage = {{.cell_index = 0U, .node_id = 11,
                              .q_request = q, .priority_weight = 1.0}},
        .surface_river = {{.cell_index = 1U, .location_id = 5,
                           .q_request = q, .priority_weight = 1.0, .q_return = q}},
        .river_lateral_ids = {{.location_id = 5, .native_lateral_id = "lat1"}},
    };

    const auto initial = state.compute_system_mass(1.0e-6);
    for (int step = 0; step < 1000; ++step) {
        const auto report = advance_tri_coupling_step(state, swmm, dflowfm, config, kDtSub);
        EXPECT_NEAR(report.surface_mass_before.surface_mass, initial.surface_mass, 1.0e-9);
        EXPECT_NEAR(report.surface_mass_after.surface_mass, initial.surface_mass, 1.0e-9);
        EXPECT_NEAR(dflowfm.get_value("laterals/lat1/water_discharge", 0), q, 1.0e-15);
        EXPECT_NEAR(swmm.get_node_lateral_inflow(11), q, 1.0e-15);
    }

    const auto final = state.compute_system_mass(1.0e-6);
    EXPECT_NEAR(final.surface_mass, initial.surface_mass, 1.0e-9);
    EXPECT_NEAR(state.cells()[0].volume, 40.0, 1.0e-9);
    EXPECT_NEAR(state.cells()[1].volume, 40.0, 1.0e-9);
    for (const auto& cell : state.cells()) {
        for (const auto& account : cell.shared_deficit_accounts) {
            EXPECT_NEAR(account.mass_deficit_account.volume, 0.0, 1.0e-12);
        }
    }
    EXPECT_DOUBLE_EQ(swmm.elapsed_time(), 4000.0);
    EXPECT_DOUBLE_EQ(dflowfm.elapsed_time(), 4000.0);
}
