...
scau::core::Real residual_norm_squared(const scau::surface2d::MomentumResidual& residual) {
...
TEST(SurfaceHllcWaveMomentum, InternalMomentumFluxIncreasesWithNormalVelocity) {
    ...
    EXPECT_GT(
        moving_diagnostics.edges[edge_index].momentum_flux_n,
        still_diagnostics.edges[edge_index].momentum_flux_n);
    EXPECT_GT(
