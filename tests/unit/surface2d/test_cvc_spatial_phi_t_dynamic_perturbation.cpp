...
double stored_volume(
    ...
    for (std::size_t i = 0; i < state.cells.size(); ++i) {
        total += dpm_fields.cells[i].phi_t * state.cells[i].conserved.h * geometry.cell_areas[i];
    }
    return total;
}
...
double run_one_dynamic_interface_step(scau::surface2d::DpmFields dpm_fields) {
    ...
    state.cells[left_index].conserved.hu = 0.35;
...
TEST(CvcSpatialPhiTDynamicPerturbation, QuantifiesCurrentStorageResidualAcrossSpatialJump) {
    ...
    auto jumped = scau::surface2d::DpmFields::for_mesh(mesh);
    const auto edge_index = first_internal_edge_index(mesh);
    const auto left_index = cell_index_by_id(mesh, *mesh.edges[edge_index].left_cell);
    const auto right_index = cell_index_by_id(mesh, *mesh.edges[edge_index].right_cell);
    uniform.cells[left_index].phi_t = 1.0;
    uniform.cells[right_index].phi_t = 1.0;
