...
scau::coupling::core::ExchangeCellState make_scheduler_control_request_ordering_cell() {
    return {
        .volume = 40.0,
        .phi_t = 0.4,
        .h = 2.0,
...
std::vector<scau::coupling::core::SharedExchangeIntent> make_scheduler_control_request_ordering_intents() {
    return {
        {
            .endpoint = {
...
scau::coupling::core::FaultControllerSchedulerControlRequest make_review_required_scheduler_control_request() {
...
TEST(CouplingFaultControllerSchedulerControlRequestOrdering, RequestBeforeSchedulerDoesNotChangeSchedulingReplayOrAudit) {
    const std::vector<scau::coupling::core::ExchangeCellState> initial_cells{make_scheduler_control_request_ordering_cell()};
    scau::coupling::core::CouplingState control{initial_cells};
    scau::coupling::core::CouplingState request_recorded{initial_cells};

    const auto control_snapshot = control.snapshot();
    const auto request_recorded_snapshot = request_recorded.snapshot();

    const auto request = make_review_required_scheduler_control_request();

    const auto control_step = control.run_mock_coupling_scheduler_step(
        0U,
        make_scheduler_control_request_ordering_intents(),
        4.0);
    const auto request_recorded_step = request_recorded.run_mock_coupling_scheduler_step(
        0U,
        make_scheduler_control_request_ordering_intents(),
        4.0);
    ...
    expect_same_decisions(control_step.decisions, request_recorded_step.decisions);

    ASSERT_EQ(control.cells().size(), request_recorded.cells().size());
    EXPECT_DOUBLE_EQ(control.cells()[0].volume, request_recorded.cells()[0].volume);
    EXPECT_DOUBLE_EQ(
        control.cells()[0].mass_deficit_account.volume,
        request_recorded.cells()[0].mass_deficit_account.volume);
    expect_same_shared_deficit_accounts(
        control.cells()[0].shared_deficit_accounts,
        request_recorded.cells()[0].shared_deficit_accounts);
    EXPECT_DOUBLE_EQ(control.cells()[0].h, request_recorded.cells()[0].h);

    const auto control_delta = control.audit_system_mass_against_snapshot(control_snapshot, 0.01);
    const auto request_recorded_delta = request_recorded.audit_system_mass_against_snapshot(request_recorded_snapshot, 0.01);
    EXPECT_DOUBLE_EQ(control_delta.baseline.total_mass, request_recorded_delta.baseline.total_mass);
    EXPECT_DOUBLE_EQ(control_delta.current.total_mass, request_recorded_delta.current.total_mass);
    EXPECT_DOUBLE_EQ(control_delta.residual, request_recorded_delta.residual);
    EXPECT_EQ(control_delta.conserved, request_recorded_delta.conserved);
}
