...
TEST(CouplingCoreState, RuntimeControlAbortPredicateAgainstSnapshotContinuesWhenMassIsConserved) {
    ...
    const bool should_abort = state.should_abort_system_mass_runtime_control_against_snapshot(baseline, kHWet);
...
TEST(CouplingCoreState, RuntimeGateAgainstSnapshotMatchesGateActionWhenConserved) {
    ...
    const auto outcome = state.evaluate_system_mass_runtime_gate_against_snapshot(baseline, kHWet);
    const auto gate_decision = state.decide_system_mass_gate_action_against_snapshot(baseline, kHWet);

    EXPECT_EQ(outcome.decision.action, gate_decision.action);
    EXPECT_EQ(outcome.decision.diagnostic.status, gate_decision.diagnostic.status);
    EXPECT_DOUBLE_EQ(outcome.decision.diagnostic.residual, gate_decision.diagnostic.residual);
    EXPECT_EQ(outcome.decision.action, scau::coupling::core::SystemMassGateAction::continue_run);
    EXPECT_EQ(outcome.status, scau::coupling::core::SystemMassRuntimeGateStatus::running);
}
...
TEST(CouplingCoreState, GateActionAgainstReferenceMatchesDiagnoseWhenConserved) {
    ...
    const auto diagnostic = state.diagnose_system_mass_against_reference(baseline, kHWet);
    const auto gate_decision = state.decide_system_mass_gate_action_against_reference(baseline, kHWet);
    const auto via_diagnostic = scau::coupling::core::decide_system_mass_gate_action(diagnostic);

    EXPECT_EQ(gate_decision.action, via_diagnostic.action);
    EXPECT_EQ(gate_decision.diagnostic.status, via_diagnostic.diagnostic.status);
    EXPECT_DOUBLE_EQ(gate_decision.diagnostic.residual, via_diagnostic.diagnostic.residual);
    EXPECT_DOUBLE_EQ(gate_decision.diagnostic.baseline_total_mass, via_diagnostic.diagnostic.baseline_total_mass);
...
TEST(CouplingCoreState, RuntimeGateAgainstReferenceMatchesAuditChainWhenConserved) {
    ...
    const auto audit = state.audit_system_mass_against_reference(baseline, kHWet);
    const auto outcome = state.evaluate_system_mass_runtime_gate_against_reference(baseline, kHWet);
    const auto via_audit = scau::coupling::core::make_system_mass_runtime_gate_outcome(
        scau::coupling::core::decide_system_mass_gate_action(
            scau::coupling::core::make_system_mass_conservation_diagnostic(audit)));

    EXPECT_EQ(outcome.status, via_audit.status);
    EXPECT_EQ(outcome.decision.action, via_audit.decision.action);
    EXPECT_EQ(outcome.decision.diagnostic.status, via_audit.decision.diagnostic.status);
    EXPECT_DOUBLE_EQ(outcome.decision.diagnostic.residual, via_audit.decision.diagnostic.residual);
    EXPECT_DOUBLE_EQ(outcome.decision.diagnostic.baseline_total_mass, via_audit.decision.diagnostic.baseline_total_mass);
    EXPECT_DOUBLE_EQ(outcome.decision.diagnostic.current_total_mass, via_audit.decision.diagnostic.current_total_mass);
    EXPECT_EQ(outcome.status, scau::coupling::core::SystemMassRuntimeGateStatus::running);
}
...
TEST(CouplingCoreState, RuntimeGateAgainstSnapshotMatchesAuditChainWhenConserved) {
    ...
    const auto audit = state.audit_system_mass_against_snapshot(snap, kHWet);
    const auto outcome = state.evaluate_system_mass_runtime_gate_against_snapshot(snap, kHWet);
    const auto via_audit = scau::coupling::core::make_system_mass_runtime_gate_outcome(
        scau::coupling::core::decide_system_mass_gate_action(
            scau::coupling::core::make_system_mass_conservation_diagnostic(audit)));
...
TEST(CouplingCoreState, RuntimeControlAgainstReferenceMatchesAuditChainWhenConserved) {
    ...
    const auto audit = state.audit_system_mass_against_reference(baseline, kHWet);
    const auto control = state.decide_system_mass_runtime_control_against_reference(baseline, kHWet);
    const auto via_audit = scau::coupling::core::make_system_mass_runtime_control_decision(
        scau::coupling::core::make_system_mass_runtime_gate_outcome(
            scau::coupling::core::decide_system_mass_gate_action(
                scau::coupling::core::make_system_mass_conservation_diagnostic(audit))));

    EXPECT_EQ(control.gate_outcome.status, via_audit.gate_outcome.status);
    EXPECT_EQ(control.gate_outcome.decision.action, via_audit.gate_outcome.decision.action);
    EXPECT_EQ(control.gate_outcome.decision.diagnostic.status, via_audit.gate_outcome.decision.diagnostic.status);
    EXPECT_DOUBLE_EQ(control.gate_outcome.decision.diagnostic.residual, via_audit.gate_outcome.decision.diagnostic.residual);
    EXPECT_DOUBLE_EQ(control.gate_outcome.decision.diagnostic.baseline_total_mass, via_audit.gate_outcome.decision.diagnostic.baseline_total_mass);
    EXPECT_DOUBLE_EQ(control.gate_outcome.decision.diagnostic.current_total_mass, via_audit.gate_outcome.decision.diagnostic.current_total_mass);
    EXPECT_EQ(control.handling_state, via_audit.handling_state);
    EXPECT_EQ(control.should_abort, via_audit.should_abort);
    EXPECT_EQ(control.gate_outcome.status, scau::coupling::core::SystemMassRuntimeGateStatus::running);
    EXPECT_EQ(control.handling_state, scau::coupling::core::SystemMassRuntimeAbortHandlingState::continue_run);
    EXPECT_FALSE(control.should_abort);
}
...
TEST(CouplingCoreState, RuntimeControlAgainstSnapshotMatchesAuditChainWhenConserved) {
    ...
    const auto audit = state.audit_system_mass_against_snapshot(snap, kHWet);
    const auto control = state.decide_system_mass_runtime_control_against_snapshot(snap, kHWet);
    const auto via_audit = scau::coupling::core::make_system_mass_runtime_control_decision(
        scau::coupling::core::make_system_mass_runtime_gate_outcome(
            scau::coupling::core::decide_system_mass_gate_action(
                scau::coupling::core::make_system_mass_conservation_diagnostic(audit))));
