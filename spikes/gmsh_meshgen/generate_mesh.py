"""M287-A gmsh mesh-generation spike: GeoJSON constraints -> STCF v5 UGRID case.

Standalone spike (spike-spec section 2 isolation): no main-repo linkage; the
only downstream contract is the on-disk .stcf.nc file, which the authoritative
validator (`scau_preproc validate`) must accept.

Fail-closed behavior (M287 plan section 5 / geometry_clean_policy):
- self-intersecting or invalid input polygons are rejected BEFORE meshing,
  with a diagnostic GeoJSON naming the offending feature (exit 2);
- any gmsh failure aborts with a diagnostic GeoJSON of every constraint in
  the active model (exit 2);
- output is written to a temporary file and renamed only after the full
  topology/field assembly succeeded (no partial mesh output);
- the caller (run_spike.py) enforces the wall-clock timeout by running this
  script as a subprocess and killing it (exit 3 path, no output file).

GPL note: gmsh (GPL-2.0+) is imported ONLY inside this subprocess. The
process boundary is the license boundary; nothing links gmsh into the
project's C++ core or the orchestration layer.
"""
