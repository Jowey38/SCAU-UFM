...
| `docs/superpowers/specs/2026-06-29-g10b-replay-drift-audit-evidence.md` | G10b Replay Drift Audit 证据；在现有 G10 Golden 内增加多轮 `rollback -> replay` 零漂移审计，要求 aggregate/shared deficit、runtime counters、system mass audit 与 pending queue 状态逐轮完全一致。 |
...
| `docs/superpowers/specs/2026-06-26-m250-cvc-dynamic-phi-t-jump-evidence.md` | M250 CVC 动态 `phi_t` 时间跳变重映射证据；新增显式 `remap_state_for_phi_t_change` seam，守恒 `phi_t*h*A` 与 `phi_t*hu/hv*A`，保持 `eta-h` 床面一致性并 fail-closed；不自动接入热路径，不宣称完整 augmented-HLLC。 |
| `docs/superpowers/specs/2026-06-26-m251-cvc-dynamic-phi-t-golden-candidate.md` | M251 CVC dynamic `phi_t` GoldenSuite 候选证据；将 `test_dpm_phi_t_remap` 标记为 `golden_candidate;cvc;surface2d`，提出独立 G13 `cvc_dynamic_phi_t_remap` 迁移方案，避免混淆既有 G2 静态空间 `phi_t` 跳变语义。 |
| `docs/superpowers/specs/2026-06-26-m252-cvc-spatial-phi-t-discontinuity-design.md` | M252 CVC spatial `phi_t` discontinuity 设计；定义非零速度空间孔隙率阶跃的 augmented-HLLC/CVC 研发路线，要求先做 failure-revealing G14 候选和纯函数 correction，再考虑默认关闭的 step wiring。 |
...
| `spikes/dflowfm/evidence/m274_native_water_balance_feasibility.md` | M274 native water-balance bridge 可行性证据；固定宽度 C ABI 与 Fortran bind(C) bridge 编译成功，确认必须编入外部 dflowfm.dll；当前 Conan/PETSc/Cygwin 构建环境不能产出 governed runtime，故 case matrix 未执行、G19/G27 保持 BLOCKED。（已由 contract 证据推进） |
